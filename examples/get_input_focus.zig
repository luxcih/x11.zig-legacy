...
pub fn main(init: std.process.Init) !void {
    ...
    try state.connection.writeAll(
        init.io,
        try x11.Input.GetInputFocus.encode(&request_buffer, state.byte_order),
    );
    ...
    const reply = try x11.Input.GetInputFocus.Reply.parse(&reply_bytes, state.byte_order);
